// http://wowjp.net/forum/189-251364-1
#include "ScriptPCH.h"
#include "Group.h"
#include "Player.h"
#include <cstring>
...
#define TOKEN_ID 38186 // token id
...
#endif
#if SET_CURRENCY == 2
#define BOUNTY_1 "Chtel bych dat odmenu 1 Event marku."
#define BOUNTY_2 "Chtel bych dat odmenu 3 Event marky."
#define BOUNTY_3 "Chtel bych dat odmenu 5 Event marek."
#define BOUNTY_4 "Chtel bych dat odmenu 10 Event marek."
#endif
 
#define PLACE_BOUNTY "Chtel bych dat odmenu."
#define LIST_BOUNTY "Vypis aktualnich odmen."
#define NVM "Odejit"
#define WIPE_BOUNTY "Smazat odmeny (vidi pouze GM)"
...
bool passChecks(Player * pPlayer, const char * name)
{
        ...
        if(!pBounty)
        {
                m_session->SendNotification("Tento hrac je offline nebo neexistuje!");
                return false;
        }
        QueryResult result = CharacterDatabase.PQuery("SELECT * FROM bounties WHERE guid ='%u'", pBounty->GetGUID());
        if(result)
        {
                m_session->SendNotification("Na tohoto hrace je jiz umistena odmena!");
                return false;
        }
...
void alertServer(const char * name, int msg)
{
        ...
        if(msg == 1)
        {
                message = "Odmena byla umistena na hrace ";
                message += name;
                message += ". Zabij ho a ziskas odmenu!";
        }
        else if(msg == 2)
        {
                message = "Odmena za hrace ";
                message += name;
                message += " byla vybrana!";
        }
...
bool hasCurrency(Player * pPlayer, uint32 required, int currency)
{
        ...
        switch(currency)
        {
                ...
                case 2: //tokens
                        ...
                        {
                                m_session->SendNotification("Nemas dostatek Event marek!");
                                return false;
                        }
